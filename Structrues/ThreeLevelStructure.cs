...
namespace PV_analysis.Structures
{
    internal class ThreeLevelStructure : Structure
    {
        ...
        public override InfoPackage GetDisplayInfo(bool isAll = false)
        {
            ...
            infoList.Add(new Info("母线电压", Math_Vbus.ToString() + "V"));
            infoList.Add(new Info("逆变直流侧电压", DCAC_Vinv.ToString() + "V"));
            package.Add(infoList);
            infoList = new InfoList("前级DC/DC");
            infoList.Add(new Info("模块数", DCDC.Number.ToString()));
            infoList.Add(new Info("开关频率", (DCDC.Math_fs / 1e3).ToString("f1") + "kHz"));
            infoList.Add(new Info("拓扑", DCDC.Topology.GetName()));
            package.Add(infoList);
            if (isAll)
            {
                package.AddRange(DCDC.GetComponentConfigInfo());
            }
            infoList = new InfoList("隔离DC/DC");
