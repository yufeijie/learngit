...
namespace PV_analysis.Components
{
    internal class DualModule : Semiconductor
    {
        ...
        private void CalcIGBTLoss(Curve curve, out double PTcon, out double Pon, out double Poff, out double PDcon, out double Prr)
        {
            ...
            for (int i = 1; i < curve.Data.Count; i++)
            {
                ...
                else //t1≠t2时，只有通态损耗
                {
                    ...
                    {
                        PTcon += CalcIGBT_PTcon(t1, i1, t2, i2); //计算主管通态损耗
                    }
                    else if (Function.LE(i1, 0) && Function.LE(i2, 0)) //电流都不为正时，为反并二极管通态损耗
                    {
                        PDcon += CalcIGBT_PDcon(t1, i1, t2, i2); //计算反并二极管通态损耗
                    }
                    else //电流一正一负时，既包含主管通态损耗，又包含反并二极管通态损耗
                    {
                        ...
                        {
                            PTcon += CalcIGBT_PTcon(t1, i1, z, 0);
                            PDcon += CalcIGBT_PDcon(z, 0, t2, i2);
                        }
                        else //否则i2>0，反并二极管先为导通状态
                        {
                            PDcon += CalcIGBT_PDcon(t1, i1, z, 0);
                            PTcon += CalcIGBT_PTcon(z, 0, t2, i2);
                        }
                    }
                }
            }
        ...
        private double CalcIGBT_PTcon(double t1, double i1, double t2, double i2)
        {
            //采用牛顿-莱布尼茨公式进行积分
        ...
        /// <returns>计算结果</returns>
        private double CalcIGBT_Pon(double Ion)
        {
            //根据开通电流查表得到对应损耗
        ...
        /// <returns>计算结果</returns>
        private double CalcIGBT_Poff(double Ioff)
        {
            //根据关断电流查表得到对应损耗
        ...
        /// <param name="i2">右电流</param>
        /// <returns>计算结果</returns>
        private double CalcIGBT_PDcon(double t1, double i1, double t2, double i2)
        {
