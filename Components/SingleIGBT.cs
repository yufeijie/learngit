...
namespace PV_analysis.Components
{
    ...
    internal class SingleIGBT : Semiconductor
    {
        ...
        /// <param name="i">电流波形</param>
        public void AddEvalParameters(int m, int n, double Vsw, double qZVS, Curve i)
        {
            math_Vsw_eval[m, n] = Vsw;
            checkZVSOn = true;
        ...
        /// <param name="i">电流波形</param>
        /// <param name="fs">开关频率</param>
        public void AddEvalParameters(int m, int n, double Vsw, double qZVS, Curve i, double fs)
        {
            math_Vsw_eval[m, n] = Vsw;
            checkZVSOn = true;
            math_qZVS_eval[m, n] = qZVS;
        ...
        /// <param name="i">电流波形</param>
        /// <param name="fs">开关频率</param>
        public void SetParameters(double Vsw, double qZVS, Curve i, double fs)
        {
            math_Vsw = Vsw;
            math_qZVS = qZVS;
            curve_i = i;
        ...
        private bool Validate()
        {
            ...
            //验证器件类型是否符合
            if (!Data.SemiconductorList[device].Category.Equals("IGBT")) return false;
        ...
        {
            CalcPowerLoss_IGBT(curve_i, out math_PTcon, out math_Pon, out math_Poff, out math_PDcon, out math_Prr);
        ...
        private void CalcPowerLoss_IGBT(Curve curve, out double PTcon, out double Pon, out double Poff, out double PDcon, out double Prr)
        {
            PTcon = 0;
            Pon = 0;
            Poff = 0;
            ...
            for (int i = 1; i < data.Length; i++)
            {
                ...
                else //t1≠t2时，只有通态损耗
                {
                    ...
                    {
                        PTcon += CalcPTcon_IGBT(t1, i1, t2, i2); //计算主管通态损耗
                    }
                    else if (Function.LT(i1, 0) && Function.LT(i2, 0)) //电流都为负时，为反并二极管通态损耗
                    {
                        PDcon += CalcPDcon_IGBT(t1, i1, t2, i2); //计算反并二极管通态损耗
                    }
                    else //电流不是同号时，有开关过程
                    {
                        ...
                        if (Function.GT(i1, 0)) //i1>0时，主管先为导通状态
                        {
                            if (!Function.EQ(t1, z))
                            {
                                PTcon += CalcPTcon_IGBT(t1, i1, z, 0);
                                Poff += CalcPoff_IGBT(0);
                            }
                            if (!Function.EQ(z, t2))
                            {
                                PDcon += CalcPDcon_IGBT(z, 0, t2, i2);
                            }
                        }
                        else //否则i2>0，反并二极管先为导通状态
                        {
                            if (!Function.EQ(t1, z))
                            {
                                PDcon += CalcPDcon_IGBT(t1, i1, z, 0);
                                Prr += CalcPrr_IGBT(0);
                            }
                            if (!Function.EQ(z, t2))
                            {
                                Pon += CalcPon_IGBT(0);
                                PTcon += CalcPTcon_IGBT(z, 0, t2, i2);
                            }
                        }
                    }
                }
        ...
        private double CalcPTcon_IGBT(double t1, double i1, double t2, double i2)
        {
            int id = Data.SemiconductorList[device].Id_Vce;
        ...
        /// <param name="i2">右电流</param>
        /// <returns>计算结果</returns>
        private double CalcPDcon_IGBT(double t1, double i1, double t2, double i2)
        {
        ...
        protected override void CalcCost()
        {
            ...
            //TODO 驱动需要不同
            driverCost = 31.4253; //IX2120B IXYS MOQ100 Mouser
        ...
        {
            //计算工作在最大结温时的散热器温度
            double Pmain = math_PTcon + math_Pon + math_Poff;
            double Pdiode = math_PDcon + math_Prr;
            double Tc = math_Tj_max - Math.Max(Pmain * Data.SemiconductorList[device].IGBT_RthJC, Pdiode * Data.SemiconductorList[device].Diode_RthJC);
